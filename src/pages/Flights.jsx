...
const Flights = () => {
    ...
    const [formData, setFormData] = useState({ name: '', code: '', origin: 'Jakarta (CGK)', destination: 'Jeddah (JED)', contact_info: '' });
    ...
    const handleOpenModal = (mode, item = null) => {
        setModalMode(mode);
        setCurrentItem(item);
        setFormData(item || { name: '', code: '', origin: 'Jakarta (CGK)', destination: 'Jeddah (JED)', contact_info: '' });
        setIsModalOpen(true);
    };
    ...
    const columns = [
        ...
        { header: 'Rute Default', accessor: 'origin', render: r => <span className="text-xs">{r.origin} ➝ {r.destination}</span> },

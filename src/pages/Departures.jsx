...
const Departures = () => {
    ...
    const [formData, setFormData] = useState({ package_id: '', departure_date: '', quota: 45, status: 'open', price_override: 0 });

    const handleOpenModal = (mode, item = null) => {
        setModalMode(mode);
        setCurrentItem(item);
        setFormData(item ? { ...item, departure_date: item.departure_date?.split('T')[0] } : { package_id: '', departure_date: '', quota: 45, status: 'open', price_override: 0 });
        setIsModalOpen(true);
    };
    ...
    const columns = [
        ...
        { header: 'Harga', accessor: 'price', render: r => {
            // Jika ada override, tampilkan harga khusus. Jika 0, tampilkan base price paket.
            const price = parseFloat(r.price_override) > 0 ? r.price_override : r.base_price;
            return (
                <div>
                    <div className="font-bold">{formatCurrency(price)}</div>
                    {parseFloat(r.price_override) > 0 && <span className="text-[10px] text-blue-600 bg-blue-50 px-1 rounded">Harga Khusus</span>}
                </div>
            )
        }},
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Atur Jadwal">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="bg-green-50 p-3 rounded border border-green-200">
                        <label className="label text-green-800">Harga Khusus (Opsional)</label>
                        <input type="number" className="input-field" placeholder="0 (Gunakan harga paket)" value={formData.price_override} onChange={e => setFormData({...formData, price_override: e.target.value})} />
                        <p className="text-xs text-green-600 mt-1">*Isi jika harga tanggal ini berbeda dengan harga standar paket.</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="label">Kuota</label><input type="number" className="input-field" value={formData.quota} onChange={e => setFormData({...formData, quota: e.target.value})} /></div>
                        <div><label className="label">Status</label><select className="input-field" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}><option value="open">Open</option><option value="full">Full</option><option value="closed">Closed</option></select></div>
                    </div>
